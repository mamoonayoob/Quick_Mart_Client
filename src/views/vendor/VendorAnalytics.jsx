...
const VendorAnalytics = () => {
  ...
  const mockSalesData = useMemo(() => ({
    ...
    datasets: [
      {
        label: 'Revenue',
        data: [5200, 6100, 5800, 6300, 7200, 8100, 8500, 9200, 9800, 10500, 11200, 12500],
  ...
  const processSalesData = (salesByDay, period) => {
    ...
    // Sort by date
    const sortedData = salesByDay.sort((a, b) => new Date(a._id) - new Date(b._id));
  ...
  const processCategoryData = (salesByCategory) => {
    ...
    const labels = salesByCategory.map(item => {
      ...
      return item._id ? item._id.charAt(0).toUpperCase() + item._id.slice(1) : 'Unknown Category';
    });
    ...
    const fetchAnalyticsData = async () => {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          const processedSalesData = processSalesData(data.salesByDay, dateRange);
          setSalesData(processedSalesData);
          
          // Process category data (sales by category)
          const processedCategoryData = processCategoryData(data.salesByCategory);
        ...
        <Col lg={6} md={6} className="mb-4 mb-lg-0">
          <Card className="admin-card h-100">
            <Card.Body>
              ...
              <h3 className="card-value">${totalRevenue ? (totalRevenue / 1000).toFixed(1) : 0}k</h3>
                ...
                <tbody>
                  {topProducts && topProducts.length > 0 ? (
                    topProducts.map((product, index) => {
                      const percentage = totalRevenue > 0 ? ((product.totalAmount / totalRevenue) * 100).toFixed(1) : 0;
                      return (
                        <tr key={product._id || index}>
                          <td>{product.name || 'Unknown Product'}</td>
                          <td>${product.totalAmount && product.totalQuantity ? (product.totalAmount / product.totalQuantity).toFixed(0) : 0}</td>
                          <td>{product.totalQuantity || 0}</td>
                          <td>${product.totalAmount ? product.totalAmount.toLocaleString() : 0}</td>
                          <td>{percentage}%</td>
                        </tr>
                      );
                    })
                    ...
                    <tr>
                      <td colSpan="5" className="text-center text-muted py-4">
                        {loading ? 'Loading products...' : 'No products data available for selected period'}
                      </td>
                    </tr>
                  )}
                </tbody>
