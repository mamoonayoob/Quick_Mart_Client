...
const DeliveryDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (allDeliveriesRes.data?.success) {
        ...
        if (statusFilter !== "all") {
          filteredDeliveries = activeDeliveries.filter(
        ...
        if (backendStats) {
          ...
          const todayDeliveries = allDeliveries.filter((delivery) => {
            const deliveryDate = new Date(
              delivery.deliveryTime || delivery.updatedAt
            )
              .toISOString()
              .split("T")[0];
          ...
          const rejectedDeliveries = allDeliveries.filter(
            (d) => d.status === "rejected"
          );
          ...
          const totalEarnings = completedDeliveries.reduce((sum, d) => {
            const amount =
              d.deliveryFee || d.earnings || (d.order?.totalPrice || 0) * 0.1;
        ...
        throw new Error(
          allDeliveriesRes.data?.message || "Failed to load deliveries"
        );
      }
    } catch (error) {
      if (error.response?.status === 401) {
        ...
        setError(
          "Access denied. You do not have permission to view delivery data."
        );
      } else {
        setError(
  ...
  const handleExport = () => {
    try {
      ...
      const dataToExport =
        filteredDeliveries.length > 0 ? filteredDeliveries : deliveries;
  ...
  const checkForPendingAssignments = async () => {
    try {
      ...
      if (response.data?.success) {
        ...
        console.log(
          "Tasks with assigned status:",
          allTasks.filter((d) => d.status === "assigned")
        );
        console.log(
          "Tasks not accepted:",
          allTasks.filter((d) => !d.isAccepted)
        );

        const assignments = allTasks.filter(
          (delivery) => delivery.status === "assigned" && !delivery.isAccepted
        );
          ...
          const newAssignments = assignments.filter(
            (assignment) =>
              !pendingAssignments.find(
                (pending) => pending._id === assignment._id
              )
          );
  ...
  const handleRejectAssignment = async () => {
    ...
    try {
      ...
      const response = await deliveryApi.rejectTask(
        currentAssignment._id,
        "Rejected by delivery person"
      );
    ...
    <div className="delivery-dashboard container mt-5 px-3 px-md-4 specialcss">
        ...
        <div className="col-12 col-lg-8 mb-3 mb-lg-0">
          ...
          <div className="d-flex flex-column flex-md-row align-items-start align-items-md-center gap-2">
            <p className="text-muted mb-0 small">
              Welcome back to your delivery portal
            </p>
            {lastUpdated && (
              <div
      ...
      <div className="bg-light shadow-sm mb-3 pb-3 rounded-bottom w-100">
        <Row className={`g-3 mx-0 ${loading ? "data-refresh" : ""}`}>
          <Col xs={12} sm={6} lg={3}>
            <div
              className="stats-card primary h-100 d-flex flex-column justify-content-between rounded shadow-sm p-3"
              style={{ boxShadow: "0 4px 12px rgba(54, 162, 235, 0.3)" }}
            >
          ...
          <Col xs={12} sm={6} lg={3}>
            <div
              className="stats-card bg-danger text-white h-100 d-flex flex-column justify-content-between rounded shadow-sm p-3"
              style={{
          ...
          <Col xs={12} sm={6} lg={3}>
            <div
              className="stats-card warning bg-warning h-100 d-flex flex-column justify-content-between rounded shadow-sm p-3"
          ...
          <Col xs={12} sm={6} lg={3}>
            <div
              className="stats-card danger bg-success h-100 d-flex flex-column justify-content-between rounded shadow-sm p-3"
              style={{ boxShadow: "0 4px 12px rgba(40, 167, 69, 0.3)" }}
            >
              <div className="stats-icon">
                <BsCheckCircle />
              </div>
      ...
      <Card
        className="admin-card mb-4 overflow-hidden w-100"
        style={{ marginTop: "0px", position: "relative", zIndex: 1 }}
      >
          ...
          {selectedDelivery && (
            <div className="mt-3">
              ...
              <p>
                ...
                {selectedDelivery.order?.user?.name || "N/A"}
              </p>
              <p>
                <strong>Delivery Address:</strong>{" "}
                {selectedDelivery.order?.shippingAddress?.street || "N/A"}
              </p>
            </div>
          )}
        </Modal.Body>
          ...
          {selectedOrder && (
            <div>
                    ...
                    <Card.Body>
                      ...
                      <div className="mb-2">
                        ...
                        <span className="ms-2 text-info">
                          {formatCurrency(
                            selectedOrder.order?.deliveryFee || 0
                          )}
                        </span>
                      </div>
                      <div className="mb-2">
                  ...
                  <Card className="h-100">
                    ...
                    <Card.Body>
                      <div className="d-flex align-items-center mb-3">
                        <img
                          src={`https://ui-avatars.com/api/?name=${encodeURIComponent(
                            selectedOrder.order?.user?.name || "Customer"
                        ...
                        <div>
                          ...
                          <small className="text-muted">
                            {selectedOrder.order?.user?.email || "N/A"}
                          </small>
                        </div>
                      </div>
                      <div className="mb-2">
                        <strong>Phone:</strong>
                        <span className="ms-2">
                          {selectedOrder.order?.user?.phone || "N/A"}
                        </span>
                        {selectedOrder.order?.user?.phone && (
                          <Button
                            ...
                            onClick={() => {
                              window.open(
                                `tel:${selectedOrder.order.user.phone}`,
                                "_self"
                              );
              ...
              {selectedOrder.order?.orderItems &&
                selectedOrder.order.orderItems.length > 0 && (
                  <Card className="mb-4">
                    <Card.Header className="bg-light">
                      <h6 className="mb-0">
                        Order Items ({selectedOrder.order.orderItems.length})
                      </h6>
                    </Card.Header>
                    <Card.Body>
                      <div className="table-responsive">
                        <table className="table table-sm">
                          ...
                          <tbody>
                            {selectedOrder.order.orderItems.map(
                              (item, index) => (
                                <tr key={index}>
                                  <td>
                                    <div className="d-flex align-items-center">
                  ...
                  {selectedOrder.order?.shippingAddress ? (
                    <div>
                      <div className="mb-3">
                        ...
                        <div className="text-muted">
                          {selectedOrder.order.shippingAddress.city},{" "}
                          {selectedOrder.order.shippingAddress.state}{" "}
                          {selectedOrder.order.shippingAddress.zipCode}
                        </div>
                    ...
                    <div className="text-center py-4">
                      ...
                      <p className="text-muted mb-0">
                        Address information is not provided for this order.
                      </p>
                    </div>
                  )}
                </Card.Body>
            ...
            <div>
              <Alert variant="info" className="mb-4">
                <div className="d-flex align-items-center">
                  ...
                  <div>
                    ...
                    <br />
                    <small>
                      Please review the details below and choose to accept or
                  ...
                  <Row>
                    <Col md={6}>
                      ...
                      <div className="mb-3">
                        ...
                        <div className="text-success fw-bold">
                          {formatCurrency(
                            currentAssignment.order?.totalPrice || 0
                          )}
                        </div>
                      </div>
                      <div className="mb-3">
                        ...
                        <div className="text-info fw-bold">
                          {formatCurrency(
                            currentAssignment.order?.deliveryFee || 0
                          )}
                        </div>
                      </div>
                    </Col>
                    <Col md={6}>
                      <div className="mb-3">
                        <strong>Customer:</strong>
                        <div>
                          {currentAssignment.order?.user?.name || "Customer"}
                        </div>
                        <small className="text-muted">
                          <BsPhone className="me-1" size={12} />
                ...
                <Card.Body>
                  {currentAssignment.order?.shippingAddress ? (
                    <div>
                      ...
                      <div className="text-muted">
                        {currentAssignment.order.shippingAddress.city},{" "}
                        {currentAssignment.order.shippingAddress.state}{" "}
                        {currentAssignment.order.shippingAddress.zipCode}
                      </div>
                      <div className="mt-2">
                        <Button
                          variant="outline-primary"
              ...
              {pendingAssignments.length > 1 && (
                <Alert variant="warning">
                  ...
                  <strong>Multiple Assignments:</strong> You have{" "}
                  {pendingAssignments.length} pending assignments. This is
                  assignment 1 of {pendingAssignments.length}.
                </Alert>
              )}
              ...
              <div className="d-flex gap-3 justify-content-center">
                ...
                >
                  {processingAssignment ? (
                    ...
                    <>
                      <BsCheckCircle className="me-2" />❌ Reject Assignment
                    </>
                  )}
                </Button>
                <Button
                ...
                >
                  {processingAssignment ? (
                    ...
                    <>
                      <BsCheckCircle className="me-2" />✅ Accept Assignment
                    </>
                  )}
                </Button>
              </div>
