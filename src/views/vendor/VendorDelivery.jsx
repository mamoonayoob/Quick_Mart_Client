  ...
  const fetchDeliveryData = useCallback(async () => {
    ...
    try {
      ...
      if (ordersResponse.data && ordersResponse.data.success) {
        ...
        console.warn(
          "Failed to fetch orders data, using mock data as fallback"
        );
        setPendingOrders(ordersResponse.data.data || []);
      }
      ...
      if (personnelResponse.data && personnelResponse.data.success) {
        ...
        console.warn(
          "Failed to fetch personnel data, using mock data as fallback"
        );
        setDeliveryPersonnel(personnelResponse.data.data || []);
      }
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        const deliveryPerson = deliveryPersonnel.find(
          (p) => p._id === selectedDeliveryPerson
        );
        const orderNumber =
          selectedOrder.orderNumber || selectedOrder.orderId || "N/A";
        ...
        console.log(
          `Successfully assigned order #${selectedOrder.orderNumber} to delivery person ${deliveryPerson?._id}`
        );
      } else {
        throw new Error(
          response.data?.message ||
            "Failed to assign delivery - no success status in response"
        );
      }
    } catch (err) {
          ...
          <Card className="admin-card h-100">
            <Card.Body className="d-flex align-items-center">
              ...
              <div className="ms-3">
                ...
                <h3 className="stats-value">{deliveryPersonnel.length}</h3>
              </div>
            </Card.Body>
          </Card>
        </Col>
