...
const Dashboard = () => {
  ...
  // State for dashboard data - initialize with mock data to prevent blank screen
  ...
  useEffect(() => {
    const fetchSalesData = async () => {
      ...
      try {
        ...
        // Ensure we have a valid data structure for the chart
        if (response?.data?.data) {
          // If the API returns the expected format
          if (response.data.data.labels && Array.isArray(response.data.data.labels)) {
            setSalesData(response.data.data);
          } 
          // If the API returns an array of sales items
          else if (Array.isArray(response.data)) {
            const labels = response.data.map(item => item.period || item.date || '');
            const data = response.data.map(item => item.amount || item.sales || 0);
  ...
  useEffect(() => {
    const fetchCategoryData = async () => {
      ...
      try {
        ...
        console.log('Category sales API response:', response?.data?.data);
        ...
        if (response?.data?.data) {
          // If the API returns the expected format with labels and datasets
          if (response.data.data.labels && Array.isArray(response.data.data.labels)) {
            chartData.labels = response.data.data.labels;
            if (response.data.data.datasets && response.data.data.datasets[0] && Array.isArray(response.data.data.datasets[0].data)) {
              chartData.datasets[0].data = response.data.data.datasets[0].data;
            }
          } 
          // If the API returns an array of category items
          else if (Array.isArray(response.data)) {
            chartData.labels = response.data.map(item => item.categoryName || item.category || 'Unknown');
            chartData.datasets[0].data = response.data.map(item => item.totalSales || item.sales || 0);
          }
        }
        
        console.log('Processed category chart data:', chartData);
  ...
  useEffect(() => {
    const fetchRecentOrders = async () => {
      ...
      try {
        ...
        console.log('Recent orders API response:', response?.data?.data);
        
        // Ensure we have an array for the DataTable
        if (response?.data?.data) {
          if (Array.isArray(response.data.data)) {
            ...
            console.warn('Unexpected data structure for recent orders:', response.data);
            setRecentOrders([]);
          }
        } else {
          setRecentOrders([]);
  ...
  useEffect(() => {
    const fetchTopProducts = async () => {
      ...
      try {
        ...
        console.log('Top products API response:', response?.data?.data);
        ...
        if (response?.data?.data) {
          if (Array.isArray(response.data.data)) {
            ...
            // Fallback to empty array if data structure is unexpected
            console.warn('Unexpected data structure for top products:', response.data);
            setTopProducts([]);
          }
        } else {
          setTopProducts([]);
